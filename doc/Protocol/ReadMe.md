...
由于网关设备相对与服务器设备的内存普遍不高,不能处理太多的负载数据，因此,下发给网关的负载数据长度最大不能超过128字节。
...
当出现异常时,根据实际情况可不包含寄存器标志与寄存器值字段。若返回值不为0，payload可只包含返回值与寄存器地址。

寄存器值为整数，小端模式。若寄存器不存在,应当在返回值中标出错误。
...
寄存器值为整数，小端模式。寄存器值表示写入后的值，通常与写入值相同,若寄存器不存在,应当在返回值中标出错误。
...
当出现异常时,根据实际情况可不包含传感器标志与传感器值字段。若返回值不为0，payload可只包含返回值与传感器地址。

传感器值为整数,小端模式。若传感器不存在,应当在返回值中标出错误。
